...
/*   Updated: 2017/10/19 13:34:56 by fmaury           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void	ft_burning_ship_2(t_env *env, int x, double zoom_x, double zoom_y)
{
	double	c_r;
	double 	c_i;
	double 	z_r;
	...
	while (y < HEIGHT)
	{
		...
		while (1)
		{
			...
			z_rt = z_r * z_r;
			z_it = z_i * z_i;
			z_r = fabs(z_r) * ((z_rt * z_rt) - 10 * (z_rt * z_it) + 5 * (z_it * z_it)) + c_r;
			z_i = fabs(z_i) * (5 * (z_rt * z_rt) - 10 * (z_rt * z_it) + (z_it * z_it)) + c_i;
			i++;
...
void	ft_burning_ship(t_env *env, int x, int xmax)
{
	...
	{
		ft_burning_ship_2(env, x, zoom_x, zoom_y);
		x++;
	}
}
